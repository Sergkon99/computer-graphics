...
def move_on(p: Point, x: float, y: float):
    """
    Смещение на (x, y) в декартовых координатах
    :param: x, y смещение
...
def reflection(p: Point, x: bool=False, y:bool=False):
    """
    Выполнит отражение точки относитьльно осей Ox, Oy или прямой X=Y
    :param: p Точка
    :param: x, : Показывает как производит отображение: 
...
def scale(p: Point, x: float=1, y: float=1):
    ...
    :param: x, y Коэффиценты масшnабирования по осям соответсвенно
...
def rotate(p: Point, angle: float):
    """
    Метод для поворота точки на угол
    :param: p точка
    :param: angle угол
    :return: Точка-результат
    """
...
def rotate_from(p: Point, angle: float, x: Point):
    """
    Метод для поворота точки на угол относитьльно другой точки
    :param: p точка
    :param: angle угол
